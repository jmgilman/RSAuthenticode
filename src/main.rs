...
use bindings::Windows::Win32::Security::Cryptography::Core::*;
...
fn main() {
    let info = AuthenticodeData::new(r#"C:\Users\Josh\Downloads\mpam-feX64.exe"#).unwrap();
    let subj_cert = info.read_subject_cert();
    let issue_cert = info.read_issuer_cert();
    println!("{:#?}", subj_cert);
    println!("{:#?}", issue_cert);
}

struct AuthenticodeData {
    data: WINTRUST_DATA,
...
impl AuthenticodeData {
    ...
    fn read_issuer_cert(&self) -> CertData {
        CertData {
    ...
    fn read_subject_cert(&self) -> CertData {
        CertData {
